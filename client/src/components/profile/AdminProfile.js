...
const AdminProfile = () => {
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      ...
      const updatePayload = {
        ...profile,
        email: undefined // Remove email from update payload
      };
  ...
  return (
    <motion.div
      ...
      initial="hidden"
      animate="visible"
      className="min-h-screen bg-gradient-to-br from-[#050505] to-[#1a1a1a] p-6"
    >
      <motion.div variants={itemVariants} className="max-w-3xl mx-auto">
        <div className="mb-8">
          <motion.h1 
            className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-500 flex items-center"
            initial={{ opacity: 0, y: -20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5, delay: 0.2 }}
          >
            <UserCircleIcon className="w-10 h-10 mr-3 text-purple-500" />
            Admin Profile
          </motion.h1>
        </div>
        ...
        >
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center">
              <motion.div 
                className="w-20 h-20 bg-gradient-to-br from-purple-500/30 to-pink-500/30 rounded-2xl flex items-center justify-center mr-5 relative overflow-hidden"
                whileHover={{ scale: 1.05 }}
                transition={{ duration: 0.2 }}
              >
                {profile.avatar ? (
                  <img src={profile.avatar} alt="Profile" className="w-16 h-16 rounded-xl object-cover" />
                ) : (
